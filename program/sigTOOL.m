...
% 25.06.16 Remove reminder. Update links to irondukepublishing.com
% 25.06.16 Embed dependency on waterloo. This is in a separate folder, a
%          clone of the main repo, so we only have one copy to maintain.
...
if isempty(RunNumber)
    RunNumber=1;
%     disp('Adding sigTOOL-0.99-SNAPSHOT.jar');
%     PathToJava=strrep(which('sigTOOL'),'program/sigTOOL.m','Java/sigTOOL/target/sigTOOL-0.99-SNAPSHOT.jar');
%     javaaddpath(PathToJava);
...
switch button
    ...
    case 'Visit website'
        ...
        web('http://www.irondukepublishing.com','-browser');
end
return
end
