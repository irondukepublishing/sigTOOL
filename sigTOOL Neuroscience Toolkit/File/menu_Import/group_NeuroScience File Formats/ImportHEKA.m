...
% Copyright © The Author & King's College London 2009-2014
...
% 20.06.16  Test file: Progbar removed. Accommodates different TrDataKins
% values
...
if ~isempty(targetpath) && targetpath(end)~=filesep()
    targetpath=[targetpath, filesep()];
end

% TODO: 20.06.2016: check that sigTOOL does not double-compensate for this
...
%--------------------------------------------------------------------------
function channelnumber=LocalImportGroup(fh, thisfile, matfilename, tree, grp, grp_row, channelnumber, progbar)
%--------------------------------------------------------------------------
...
%             sprintf('Importing Channel %d', channelnumber));
    ...
    for k=1:size(tr_row, 1)
        ...
        [tr_s, isConstantScaling, isConstantFormat, isFramed, isConstantTrDataKind, dataKindArray]=LocalCheckEntries(tree, tr_row, k);
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % CHARLIE - what's happening here?
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if ~isConstantTrDataKind
            % The first sweep is different
            fprintf('TrDataKind was not constant for series %d at row %d\n', ser, k);
            if numel(unique(dataKindArray(2:end))) == 1 
                fprintf('TrDataKind has first entry of: %d.\n', dataKindArray(1));
                fprintf('Subsqequent entries have values of %d.\n', dataKindArray(2));
                
                fprintf('<strong>Value of %d indicates:\n</strong>', dataKindArray(1));
                displayDataKindMeaning(dataKindArray(1));
                
                fprintf('<strong>Value of %d indicates:\n</strong>', dataKindArray(2));
                displayDataKindMeaning(dataKindArray(2));
            else
                % A sweep (or sweeps) other than first sweep is/are different
                fprintf('<strong>Values found:\n</strong>');
                values=unique(dataKindArray);
                for n=1:numel(values)
                    fprintf('<strong>%d:\n</strong>', values(n));
                    displayDataKindMeaning(values(n));
                end
                disp(dataKindArray);
            end
            fprintf('----------------\n');
        ...
        for tr=1:size(tr_row,2)
            ...
            if ~INTERLEAVE_SUPPORTED || tree{tr_row(k,tr),5}.TrInterleaveSize==0 
                ...
                for nread=1:floor(numel(data)/double(tree{tr_row(k,tr),5}.TrInterleaveSize/nbytes))
                    ...
                    fseek(fh, tree{tr_row(k,tr),5}.TrData+double(tree{tr_row(k,tr),5}.TrInterleaveSkip*nbytes), 'bof');
                end
            end
        end
...
%--------------------------------------------------------------------------
function [tr_s, isConstantScaling, isConstantFormat, isFramed, isConstantTrDataKind, dataKindArray]=LocalCheckEntries(tree, tr_row, k)
%--------------------------------------------------------------------------
% Check units are the same for all traces

% 11.06.2016 Change for speed
%tr_s=cell2mat({tree{tr_row(k, :),5}});
tr_s=cell2mat(tree(tr_row(k, :),5).');

% Check for conditions that are unexpected and will lead to error in the
% sigTOOL data file
isConstantTrDataKind=true;
dataKindArray=[];
if numel(unique(cell2mat({tr_s.TrDataKind})))>1
    fprintf('<strong>w1001(is this OK?): Data are of different kinds</strong>\n');
    isConstantTrDataKind=false;
    dataKindArray=cell2mat({tr_s.TrDataKind});
end
...
%--------------------------------------------------------------------------
function displayDataKindMeaning(value)
%--------------------------------------------------------------------------

if bitget(value, 1)
    disp('Computer = Windows or Intel Mac');
else
        disp('Computer = PowerPC Mac: Big-endian');
end
if bitget(value, 2)
    disp('Leak Channel = true');
else
    disp('Leak Channel = false')
end
if bitget(value, 3)
    disp('Virtual Channel = true');
else
    disp('Virtual Channel = false')
end
if bitget(value, 4)
    disp('Trace from Imon ADC = true');
else
    disp('Trace from Imon ADC = false')
end
if bitget(value, 5)
    disp('Trace from Vmon ADC = true');
else
    disp('Trace from Vmon ADC = false')
end
if bitget(value, 6)
    disp('Amplifier of trace was clipping = true');
else
    disp('Amplifier of trace was clipping = false')
end
end
